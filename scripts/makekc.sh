...
/usr/bin/security create-keychain -p "" $KC_NAME
/usr/bin/security default-keychain -s $KC_NAME
/usr/bin/security unlock-keychain -p "" $KC_NAME
/usr/bin/security list-keychains -d user -s $KC_NAME

echo ">> Importing Certificate"
/usr/bin/security import $CERT_PATH -k $KC_NAME -P "" -T /usr/bin/codesign
/usr/bin/security set-key-partition-list -S apple-tool:,apple:,codesign: -s -k "" $KC_NAME
...
/usr/bin/security import AppleWWDRCAG3.cer.der -k $KC_NAME -P ""
